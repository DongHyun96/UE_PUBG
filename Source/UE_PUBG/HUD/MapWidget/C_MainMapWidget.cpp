...
void UC_MainMapWidget::NativeConstruct()
{
	...
	if (CurrentSelectedMap == ELevelType::ShantyTown)
	{
...
void UC_MainMapWidget::HandleUpdateMainMapImage(float InDeltaTime)
{
	...
	MapImage->SetRenderScale(Scale);
	GridImage->SetRenderScale(Scale);

	FVector2D MainMapPos = MapImage->GetRenderTransform().Translation;
...
void UC_MainMapWidget::HandleUpdateMarkers()
{
	...
	const FVector2D PlayerPos		= { PlayerLocation.Y, -PlayerLocation.X };
	FVector2D		PlayerMarkerPos = PlayerPos * (CANVAS_SIZE / WorldMapSize) * MainMapScale;

	//PlayerMarkerPos += MapImage->GetRenderTransform().Translation * (1 / MainMapScale);
	PlayerMarkerPos += MapImage->GetRenderTransform().Translation;

	PlayerMarker->SetRenderTranslation(PlayerMarkerPos);

	if (!PingBorder)
	{
		UC_Util::Print("From UC_MainMapWidget::HandleUpdateMarkers : PingBorder nullptr");
		return;
	}

	FVector2D PingMarkerToMainMapPos = PingMarkerPos * MainMapScale;
	PingMarkerToMainMapPos += MapImage->GetRenderTransform().Translation;

	PingBorder->SetRenderTranslation(PingMarkerToMainMapPos);
...
void UC_MainMapWidget::HandleUpdatePlaneRouteTransform()
{
	...
	StartPos += MapImage->GetRenderTransform().Translation;
	AirplaneStartCircle->SetRenderTranslation(StartPos);
	AirplaneStartCircle->SetRenderScale(ImgScale);

	// Dest Triangle 이미지 위치 크기 잡기
	FVector2D DestPos = AirplaneRouteDestPos * MainMapScale;
	DestPos += MapImage->GetRenderTransform().Translation;
	AirplaneDestTriangle->SetRenderTranslation(DestPos);
	AirplaneDestTriangle->SetRenderScale(ImgScale);

	// PlaneRoute 이미지 위치 크기 잡기
	FVector2D PlaneRoutePos = (StartPos + DestPos) * 0.5f;
	AirplaneRouteLine->SetRenderTranslation(PlaneRoutePos);
	AirplaneRouteLine->SetRenderScale(FVector2D(1.f, MainMapScale));
...
FReply UC_MainMapWidget::NativeOnMouseMove(const FGeometry& InGeometry, const FPointerEvent& InMouseEvent)
{
	...
	//FVector2D MainMapPosition = MapImage->GetRenderTransform().Translation;
	...
	//MapImage->SetRenderTranslation(MainMapPosition);
...
FReply UC_MainMapWidget::NativeOnMouseWheel(const FGeometry& InGeometry, const FPointerEvent& InMouseEvent)
{
	...
	//MapImage->SetRenderScale(FVector2D(MainMapScale, MainMapScale));
...
bool UC_MainMapWidget::SpawnPingImage(FVector2D MousePos)
{
	...
	PingMarkerPos -= MapImage->GetRenderTransform().Translation;
	PingMarkerPos /= MainMapScale;

	// Spawn MainMap Ping
	PingBorder->SetVisibility(ESlateVisibility::SelfHitTestInvisible);

	FVector2D WorldPingPos2D = PingMarkerPos * (WorldMapSize / CANVAS_SIZE);

	// Landscape쪽으로 LineTrace 처리
	FCollisionQueryParams CollisionParams{};
	FHitResult HitResult{};

	const float LineTraceStartHeight = 25000.f;
	FVector LineTraceStartLocation = { -WorldPingPos2D.Y, WorldPingPos2D.X, LineTraceStartHeight };
	LineTraceStartLocation += OriginLocation;
	FVector LineTraceEndLocation = LineTraceStartLocation - FVector(0.f, 0.f, LineTraceStartHeight);
	...
	FVector WorldPingLocation = (HasHit) ? HitResult.ImpactPoint :
		FVector(-WorldPingPos2D.Y, WorldPingPos2D.X, FixedLandScapeHeight);
...
void UC_MainMapWidget::UpdateAirplaneImagePosition(const FVector& AirplaneLocation)
{
	...
	Position += MapImage->GetRenderTransform().Translation;

	AirplaneImage->SetRenderTranslation(Position);

	FVector2D AirplaneImgScale = FVector2D(1.f, 1.f) * MainMapScale;
	AirplaneImage->SetRenderScale(AirplaneImgScale);
}
...
bool UC_MainMapWidget::HandleRMBDown(const FGeometry& InGeometry, const FPointerEvent& InMouseEvent)
{
	...
	if (PingBorder->GetVisibility() != ESlateVisibility::Hidden)
	{
		FVector2D MarkedPos = MousePos - MID_POINT;
		MarkedPos -= MapImage->GetRenderTransform().Translation;
...
FVector2D UC_MainMapWidget::GetWorldToMapSizePos(FVector GameWorldLocation)
{
	...
	return Pos * (CANVAS_SIZE / WorldMapSize);
}
