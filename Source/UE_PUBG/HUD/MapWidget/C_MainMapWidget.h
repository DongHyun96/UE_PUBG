...
public:
	...
	/// <returns> : 제대로 Spawn 되었다면 return true </returns>
	bool SpawnPingImage(FVector2D MousePos);
...
private:
	...
	float WorldMapSize{};			// 현재 level Map의 가로(또는 세로축) 길이 크기 (ex) 1km -> 100,000)
	float FixedLandScapeHeight{};	// 현재 Level Map의 LandScape Z값 Height
